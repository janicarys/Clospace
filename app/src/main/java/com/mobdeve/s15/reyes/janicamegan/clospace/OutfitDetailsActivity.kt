...
class OutfitDetailsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        intent.getStringExtra(EXTRA_DATE)?.let { preset -> selectedDate = preset }
        bindSchedule(selectedDate)

        findViewById<View>(R.id.layoutDate).setOnClickListener { pickDate() }
        findViewById<View>(R.id.layoutOccasion).setOnClickListener { pickOccasion() }
    ...
    private fun loadClothes() {
        lifecycleScope.launch {
            val items = clothingIds.mapNotNull { backend.getClothingById(it) }
            bindClothes(items)
        }
    }
